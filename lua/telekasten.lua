...
local function save_all_mod_buffers()
	for i = 1, vim.fn.bufnr("$") do
		if
			...
			and (
				(M.Cfg.auto_set_filetype == true and vim.fn.getbufvar(i, "&filetype") == "telekasten")
...
local function imgFromClipboard()
	global_dir_check(function(dir_check)
		...
		paste_command["xsel"] = function(dir, filename)
			...
			if vim.fn.system("file --mime-type -b " .. image_path):gsub("%s+", "") == "image/png" then
		...
		paste_command["xclip"] = function(dir, filename)
			return "xclip -selection clipboard -t image/png -o > " .. dir .. "/" .. filename
		end
		...
		if output ~= "" then
			...
			vim.api.nvim_err_writeln(
				string.format("Unable to write image %s.\nIs there an image on the clipboard?\nSee also issue 131", png)
			)
		end
...
local function create_note_from_template(title, uuid, filepath, templatefn, calendar_info, callback)
	...
	check_dir_and_ask(file_dir, "Create weekly dir", function(dir_succeed)
		...
		for _, line in pairs(lines) do
			ofile:write(
				templates.subst_templated_values(line, title, calendar_info, uuid, M.Cfg.calendar_opts.calendar_monday)
					.. "\n"
			)
		end
...
local function check_if_daily_or_weekly(title)
	...
	local dateinfo = dateutils.calculate_dates(nil, M.Cfg.calendar_opts.calendar_monday) -- sane default
	...
	if start ~= nil then
		if tonumber(month) < 13 then
			if tonumber(day) < 32 then
				...
				dateinfo = dateutils.calculate_dates(dateinfo, M.Cfg.calendar_opts.calendar_monday)
			end
		end
	end
...
function Pinfo:resolve_link(title, opts)
	...
	self.title = title
	self.filename = title
	-- If the title doesn't have an extension, add the default one
	if not self.filename:match("%.[^%.]+$") then
		self.filename = self.filename .. opts.extension
	end
	...
	if opts.weeklies and fileutils.file_exists(opts.weeklies .. "/" .. self.filename) then
	...
	if opts.dailies and fileutils.file_exists(opts.dailies .. "/" .. self.filename) then
		-- TODO: parse "title" into calendarinfo like below
		-- not really necessary as the file exists anyway and therefore we don't need to instantiate a template
		-- if we still want calendar_info, just move the code for it out of `if self.fexists == false`.
...
local media_preview = defaulter(function(opts)
	...
	if M.Cfg.media_previewer == "catimg-previewer" then
		preview_cmd = M.base_directory .. "/telekasten.nvim/scripts/catimg-previewer"
	end

	if vim.startswith(M.Cfg.media_previewer, "viu-previewer") then
		preview_cmd = M.base_directory .. "/telekasten.nvim/scripts/" .. M.Cfg.media_previewer
	end
...
local function FindDailyNotes(opts)
	...
	global_dir_check(function(dir_check)
		...
		if
			(fexists ~= true)
			and ((opts.dailies_create_nonexisting == true) or M.Cfg.dailies_create_nonexisting == true)
		then
			create_note_from_template(today, nil, fname, M.note_type_templates.daily, nil, function()
				opts.erase = true
...
local function FindWeeklyNotes(opts)
	...
	global_dir_check(function(dir_check)
		...
		then
			create_note_from_template(title, nil, fname, M.note_type_templates.weekly, nil, function()
				opts.erase = true
				opts.erase_file = fname
				picker()
			end)
	...
	if M.Cfg.rename_update_links == true then
		...
		if #(vim.fn.getbufinfo({ bufmodified = 1 })) > 1 then
			vim.ui.select({ "Yes (default)", "No" }, {
				prompt = "Telekasten.nvim: " .. "Save all modified buffers before updating links?",
			}, function(answer)
				if answer ~= "No" then
...
local function RenameNote()
	...
	fileutils.prompt_title(M.Cfg.extension, oldfile.title, function(newname)
		...
		if M.Cfg.subdirs_in_links == true and newpath == M.Cfg.home .. "/" and oldfile.sub_dir ~= "" then
...
local function GotoDate(opts)
	opts.dates = dateutils.calculate_dates(opts.date_table, M.Cfg.calendar_opts.calendar_monday)
	opts.insert_after_inserting = opts.insert_after_inserting or M.Cfg.insert_after_inserting
	opts.close_after_yanking = opts.close_after_yanking or M.Cfg.close_after_yanking
	...
	if
		...
		and ((opts.dailies_create_nonexisting == true) or M.Cfg.dailies_create_nonexisting == true)
	then
		create_note_from_template(word, nil, fname, M.note_type_templates.daily, opts.dates, function()
			opts.erase = true
			opts.erase_file = fname
			picker()
		end)
	...
	find_files_sorted({
		...
		attach_mappings = function(prompt_bufnr, map)
			actions.select_default:replace(function()
				...
				create_note_from_template(title, uuid, fname, template, pinfo.calendar_info, function()
					-- open the new note
					vim.cmd("e " .. fname)
					picker_actions.post_open()
				end)
...
local function on_create(opts, title)
	...
	if pinfo.fexists ~= true then
		...
		create_note_from_template(title, uuid, fname, pinfo.template, pinfo.calendar_info, function()
			opts.erase = true
			opts.erase_file = fname
			picker()
		end)
...
local function CreateNoteWithCustomExtension()
	vim.ui.input({ prompt = "Title with extension (e.g., note.txt): " }, function(input)
		...
		if not title or not extension then
			print("Invalid format. Please include a file extension (e.g., note.txt).")
			return
		end
